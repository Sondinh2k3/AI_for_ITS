...
def general_multi_agent_network(parallel=True, **kwargs):
    """General network.

    Agents could have the different observation and action spaces.
    The spaces then are padded to achieve the same size for all. 
    """

    if parallel:
        general_env = parallel_env(**kwargs)
    else:
        general_env = env(**kwargs)

    general_env = ss.multiagent_wrappers.pad_observations_v0(general_env)
    general_env = ss.multiagent_wrappers.pad_action_space_v0(general_env)
    return general_env
